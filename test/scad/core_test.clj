(ns scad.core-test
  "Golden tests for kotoba.scad — the OpenSCAD hiccup. They pin that an EDN CSG tree compiles to the
...
(deftest expressions-in-args
  (is (= "cylinder(h=(h + 2), r=r);" (s/node [:cylinder {:h [:+ :h 2] :r :r}]))
      "arithmetic in a named arg (shared kotoba.expr), not a point")
